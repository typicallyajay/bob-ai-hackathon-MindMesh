      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 animate-fade-in">
        {HINTS.map(({ icon: Icon, color, fill, title, desc }, idx) => (
          <div
            ...
            style={{
              background: `linear-gradient(135deg, ${fill}08 0%, rgba(248,250,252,1) 100%)`,
            ...
            onMouseLeave={e => {
              ...
              (e.currentTarget as HTMLElement).style.borderColor = `${fill}25`;
            }}
          >
            {/* Background orb */}
            <div
            ...
            <div
              ...
              style={{ background: `${fill}20`, border: `1px solid ${fill}30` }}
            >
              <Icon className="h-4 w-4" style={{ color }} />
            </div>
            <div className="relative z-10">
              <p className="text-xs font-bold text-slate-800 mb-1">{title}</p>
              <p className="text-[11px] leading-relaxed" style={{ color: `${color}90` }}>{desc}</p>
            </div>
          </div>
        ))}
      </div>
